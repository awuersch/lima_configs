  ...
  while IFS='	' read -a line; do
    ...
    if [[ X"$debpkg" == X"$pkg" ]]; then
      if [[ -f $APTPKGS/$deb ]]; then
        log_ecs info "$pkg $version is already in $APTPKGS"
      else
        log_ecs info "pulling $pkg $version"
        ...
        if (( ${#debs[@]} > 0 )); then
          ...
          log_ecs info "stored $pkg $version and dependencies in $APTPKGS"
        else
          log_ecs error "no deb files were found for $pkg $version !!"
        fi
      fi
    else
      log_ecs debug "apt package ${pkg} depends on ${debpkg}=${version}"
    fi
  done < /tmp/xx
done
# forbid glob again
...
log_ecs info "apts are pulled and stored"
...
log_ecs info "python venv $VENV is activated."
# allow glob
set +f; shopt -s nullglob
for f in $PYPIS/json/*.tsv; do
  ...
  while IFS='	' read -a line; do
    ...
    if [[ X"$package" == X"$pkg" ]]; then
      pypi-mirror download -d $PYPIPKGS ${package}==${version} > /dev/null 2>&1
      log_ecs info "pypi ${package}==${version} and its dependencies are downloaded."
    else
      log_ecs debug "pypi package ${pkg} depends on ${package}==${version}"
    fi
  done < /tmp/xx
done
# forbid glob again
...
log_ecs info "pypis are pulled and stored"
...
log_ecs info "raws are pulled and stored"
