...
function prepare-mac-host { #
  ...
  TAR="${LIMA_HOST_DATA_DIR}/kind-${KIND_NODE_VERSION}-image.tar"
  if [[ ! -f $TAR ]]; then
...
function registries { #
  ...
  # DOCKER IMAGE CACHES ("registry v2" or "distribution:3.0.0-alpha.1")
  ...
  if [ ${registry_img_loaded} -ne 1 ]; then
    ...
    # replace forward slashes and colon by hyphens
    s=${REGISTRY_IMAGE_TAG//\//-}
    docker load < ${LIMA_HOST_DATA_DIR}/${s/:/-}.tar
  fi
